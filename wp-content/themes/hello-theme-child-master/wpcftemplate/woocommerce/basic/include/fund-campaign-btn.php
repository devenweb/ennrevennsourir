	...
	if ( $product->get_type() == 'crowdfunding' ) {
		if ( wpcf_function()->is_campaign_valid( $post->ID ) ) {
			...
			if ( is_array( $predefined_price ) && count( $predefined_price ) ) {
				echo '<ul class="wpcf_predefined_pledge_amount" style="margin-left:0px;">';
			...
			<form enctype="multipart/form-data" method="post" class="cart AA">
				...
				<button type="submit" class="<?php echo apply_filters( 'add_to_donate_button_class', 'wpneo_donate_button' ); ?>"><?php _e( 'Donate', 'wp-crowdfunding' ); ?></button>
			</form>
