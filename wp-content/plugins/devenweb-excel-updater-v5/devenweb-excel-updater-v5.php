...
function dw_excel_price_format($price)
{
    return 'Rs ' . number_format($price, 2, '.', ',');
}

function dw_excel_enqueue_scripts()
{
    if (isset($_GET['page']) && $_GET['page'] === 'excel-update') {
        // SECURITY: CDN scripts lack Subresource Integrity (SRI). For production, host locally or add integrity hashes.
...
add_action('admin_enqueue_scripts', 'dw_excel_enqueue_scripts');

function dw_excel_add_menu_page()
{
    add_menu_page(
        'Update from Excel',
        'Excel Update',
        'manage_options',
        'excel-update',
        'dw_excel_render_page',
        'dashicons-update',
        21
    );
}
add_action('admin_menu', 'dw_excel_add_menu_page');

function dw_excel_render_page()
{
    ?>
    <div class="wrap">
    ...
    <style>
        #excel-update-container {
            /* max-width: 1200px; — removed for full-width responsive layout */
            width: 100%;
...
add_action('wp_ajax_process_excel_data', 'dw_excel_handle_data');
function dw_excel_handle_data()
{
    ...
    if (!is_array($data) || empty($data)) {
        wp_send_json_error('Invalid or empty Excel data');
    }
    ...
    foreach ($data as $row) {
        ...
        // Fallback: accept 'Amount Fund Raised' or 'Fund Raised' column names
        $amount = 0;
        if (isset($row['Amount Fund Raised'])) {
            $amount = floatval($row['Amount Fund Raised']);
        } elseif (isset($row['Fund Raised'])) {
            $amount = floatval($row['Fund Raised']);
        }
        $found = false;
                ...
                foreach ($campaign_titles as $campaign_title) {
                    if (stripos($campaign_title, $bracketed_name) !== false) {
                        ...
                        $html .= '<tr>
                            ...
                            <td>' . dw_excel_price_format($total_raised ?: 0) . '</td>
                            <td>' . dw_excel_price_format($amount) . '</td>
                            <td>
...
add_action('wp_ajax_update_campaign_excel', 'dw_excel_update_campaign');
function dw_excel_update_campaign()
{
    ...
    // Validate campaign product exists before creating an order
    $product = wc_get_product($campaign_id);
    if (!$product) {
        wp_send_json_error('Campaign product not found');
    }
    ...
    // Add a custom order source tag for filtering
    $order->update_meta_data('_order_source', 'excel');
